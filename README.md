...
```bash
# 1. Initialize (creates global config at ~/.config/openbook/)
openbook init
...
The config and database are stored globally in `~/.config/openbook/`, so you can run openbook from anywhere.
